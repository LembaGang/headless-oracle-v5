...
### GAP-020 — x402 surface is v1-only until Coinbase ships a v2-capable npm client (resolved 2026-06-07, do not regress)

- **Status**: **SHIPPED + SETTLED**. PR #21, commit `12bb670`, worker `949607ae`. First CDP-settled Bazaar-indexing payment landed at block 47,022,787 / 2026-06-07T12:22:01 UTC, tx `0xa6bc45dc8a1aa8652e59ca605b5a1adc1ee4f9c6197cf52fe2c4dc8cfe87ee41`, $0.001 USDC from payer `0xa385…b079` to `0x26D4…AD3` via CDP relayer. CDP catalogue recompute on ~6h cycle; check `bazaar.x402.org` discovery for "market state" from ~18:22 UTC.
- **Symptom**: the Bazaar-indexing payment against `/v5/status/x402?mic=XNYS` failed at CDP `/verify` with `FACILITATOR_VERIFY_FAILED: invalid_network`. Same root cause silently affected the main agent payment path (`/v5/status` trial-exhausted 402 via `buildMainnetFacilitatorPayload`) — broken since commit `21f5f08` (2026-06-05) but invisible because nothing settled between then and the test.
- **Root cause**: yesterday's PR bumped three 402-emitting functions (`buildX402ScanPayload`, `buildMainnetFacilitatorPayload`, `buildX402IndexHeaders`) to `x402Version: 2` while leaving the `accepts[0]` block in v1 schema shape (`network: "base"`, `maxAmountRequired: '1000'`). The latest published x402 npm client (`x402@1.2.0`) only emits v1-shape paymentPayloads regardless of the version it's told to use — it propagates the version marker only. The worker then forwards `x402Version: 2` to CDP's `/verify`, which validates the v1-shape `paymentRequirements` against the v2 zod schema (`NetworkSchemaV2` requires CAIP-2 like `"eip155:8453"`; bare `"base"` fails the `.includes(':')` refine).
- **Whose side**: ours. v1 served all 28 days from first-dollar (`0xeb9da873`, 2026-04-03) through 2026-06-04 because CDP's v1 schema validates `network` as loose `NonEmptyString`. v2 is strictly stricter; we bumped without a v2-capable client to verify against.
- **Fix shipped (2026-06-07, PR #21)**: revert all three 402 emitters to `x402Version: 1`. Bazaar `extensions` block sits at the top level of the 402 (not inside `accepts`) so discovery indexing survives the downgrade unchanged. 13 test-body assertions flipped `2 → 1`; 3 mock-client `btoa()` payloads stay at `1` (they always simulated v1-shape client output correctly). CI: 1071/1071 green. Live-verified: `/v5/status/x402` body, `Payment-Required` header, and main `/v5/status` path all emit `x402Version: 1`.
- **Hidden win**: the revert also closed a silent break on the **main `/v5/status` revenue path**. `buildMainnetFacilitatorPayload` was advertising v2 since 2026-06-05 — every real paying agent who exhausted the 3/day trial and tried to pay would have hit the same `invalid_network` rejection. Caught before any real customer hit it because no real settlement happened between the bug landing and the test.
- **Do not regress — carry-forward rule**: when Coinbase publishes a v2-capable x402 client to npm (track upstream `x402` npm releases — `>=2.x` likely signals v2 client support), do the v2 migration as a single coordinated change: (a) CAIP-2 `network: "eip155:8453"` in both the 402 `accepts[0]` and the `paymentRequirements` constructed inside `verifyX402ViaFacilitator`, (b) rename `maxAmountRequired → amount` per `PaymentRequirementsV2Schema`, (c) move `resource/description/mimeType` out of `accepts[0]` into top-level `resource: ResourceInfoSchema`. Validate end-to-end with a real CDP settlement on the new client before flipping the version marker.
