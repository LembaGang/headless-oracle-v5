...
### GAP-020 — x402 v2 served correctly beside v1 (superseded 2026-09-07; the old "do not bump to v2" rule is withdrawn)

- **Status**: **CODE LANDED, LIVE UNVERIFIED.** Rail sprint T1, 2026-09-07. The row closes — "closed &lt;date&gt;, commit &lt;sha&gt;, tx &lt;hash&gt;" — only when a real v2 payment settles through CDP against the deployed worker. That needs the founder's deploy and a funded run; until then this row stays open however green the suite is.
...
- **Not closed.** `receipt-verify`'s `ho.receipt` adapter does not exist: version 0.1.2 declares four formats (`verification.*`, `acta.receipt/0`, `evidence.action/0`, `insight.attestation/eip712`) and none of them is HO's receipt. The DoD line "receipt-verify's ho.receipt adapter reports 0 unaddressed coverage items" cannot be run until that adapter is written, in a repository this sprint does not own. Live receipts carrying the block wait on the founder's deploy.
