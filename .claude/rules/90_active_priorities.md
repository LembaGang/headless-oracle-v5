...
### GAP-020 — x402 surface is v1-only until Coinbase ships a v2-capable npm client (logged 2026-06-07)

- **Symptom**: a real CDP-settled Bazaar-indexing payment against `/v5/status/x402?mic=XNYS` failed at CDP `/verify` with `FACILITATOR_VERIFY_FAILED: invalid_network`. Same root cause affected the main agent payment path (`/v5/status` trial-exhausted 402 via `buildMainnetFacilitatorPayload`) — silently broken since commit `21f5f08` (2026-06-05) because nothing settled between then and the test.
- **Root cause**: yesterday's PR bumped three 402-emitting functions (`buildX402ScanPayload`, `buildMainnetFacilitatorPayload`, `buildX402IndexHeaders`) to `x402Version: 2` while leaving the `accepts[0]` block in v1 schema shape (`network: "base"`, `maxAmountRequired: '1000'`). The legacy x402 npm client at the latest published version (`x402@1.2.0`) only emits v1-shape paymentPayloads regardless of the version it's instructed to use — it just propagates the version marker. The worker then forwards `x402Version: 2` to CDP's `/verify`, which validates the v1-shape `paymentRequirements` against the v2 zod schema (`NetworkSchemaV2` requires CAIP-2 like `"eip155:8453"`; bare `"base"` fails the `.includes(':')` refine).
- **Whose side**: ours. v1 served all 28 days from first-dollar (`0xeb9da873`, 2026-04-03) through 2026-06-04 because CDP's v1 schema validates `network` as loose `NonEmptyString`. The v2 schema is strictly stricter, and we unilaterally bumped to v2 without a v2-capable client to verify against.
- **Fix (2026-06-07)**: revert all three 402 emitters to `x402Version: 1`. Bazaar `extensions` block sits at the top level of the 402 (not inside `accepts`) so discovery indexing survives the downgrade unchanged. 13 test-body assertions flipped from `2 → 1`; 3 mock-client `btoa()` payloads stay at `1` (they always simulated v1-shape client output correctly).
- **Carry-forward**: when Coinbase publishes a v2-capable x402 client to npm, do the v2 migration properly in a coordinated change: (a) CAIP-2 `network: "eip155:8453"` in both the 402 `accepts[0]` and the `paymentRequirements` constructed inside `verifyX402ViaFacilitator`, (b) rename `maxAmountRequired → amount` per `PaymentRequirementsV2Schema`, (c) move `resource/description/mimeType` out of `accepts[0]` into top-level `resource: ResourceInfoSchema`. Track upstream `x402` npm releases — version `>=2.x` likely signals v2 client support.
