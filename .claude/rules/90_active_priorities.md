...
- **Status**: **CLOSED 2026-09-07.** Rail sprint T1 code (`6e2f0da`) deployed as `261c48a` and settled live the same day. A stock `@x402/fetch` 2.20.0 client — no `registerV1`, spend guard 1000 atomic units, free trial exhausted first (`trial_used: 3 / limit: 3`, four consecutive 402s) — parsed the v2 `Payment-Required`, signed one EIP-3009 authorisation and received 200 with a `PAYMENT-RESPONSE` settlement header. tx `0x46db8fc8cfd79017375d76c5ad80256950f8437ff009ecbe90b6cd5ce97e9263`, block 50,998,385, 2026-09-07T13:01:57Z, status 1, one USDC `Transfer` of 1000 atomic units from the payer to `0x26D4…AD3`. 402 header digest `a2a4219538aca031102b024a80b83489bf7ecd04b41c2abc60b703604c36170e` (502 B); settlement header digest `8ecbb71433b0019d2fd924f87dadf9be20302660f1a848e3e7dba22670f20ca3` (176 B). Total spend 0.001 USDC. Evidence: `cc-output/CC_REPORT_2026-09-07_x402-v2-rail-live.md`.
- **What the old rule said, and why it was wrong.** It said: *"Do NOT bump to v2 again until Coinbase publishes a v2-capable client to npm."* Both halves failed. (a) A v2-capable client has been on npm since 2025-12-11 — `@x402/fetch` 2.x, 2.25.0 current, the study rig pins 2.20.0 — so the precondition had been met for nine months without anyone checking. (b) More seriously, the June revert did not leave us serving a *valid v1* 402 either, so "stay on v1" was never the safe option it looked like.
- **What T0 measured (RAIL_T0_2026-09-07.md, ratified `LEAD_RATIFICATION_2026-09-07_rail-t0.md`).** A stock `@x402/fetch` 2.20.0 client, run against production `/v5/status` on 2026-09-07 with the free trial exhausted first, parsed the 402 successfully and then died at `Failed to create payment payload: No client registered for x402 version: 1` — inside `x402Client.createPaymentPayload`, before any signature and before any network matching. Spend 0 USDC.
- **Root cause, ours (B-106).** The `Payment-Required` header declared `x402Version: 1` while carrying the **v2** field name `amount` and omitting the **v1**-mandatory `resource` and `description`. It matched neither `PaymentRequirementsV1Schema` nor `PaymentRequirementsV2Schema` in `@x402/core` 2.20.0. And because `x402HTTPClient.getPaymentRequiredResponse` reads the header first and consults the body only when the header is absent (`dist/esm/chunk-4Y6I6537.mjs:1097-1105`), our schema-correct v1 *body* was never read by any current client. One 402, two representations, and they disagreed — the same class as Row 21 in the freeze pack, this time in our own product.
- **The shape ruled by the Lead, and shipped.** Not replace-or-add but fix-and-serve-both:
  - **v2 header set** — `Payment-Required` (base64 JSON) plus the non-standard `Payment-Required-Json` mirror, both a schema-valid v2 `PaymentRequired`: CAIP-2 `eip155:8453`, `amount`, and a top-level `resource` ResourceInfo. The mirror was kept rather than dropped (402-index crawlers already read it) and now carries byte-identical JSON, asserted by test.
  - **v1 body stays** — schema-valid `PaymentRequiredV1Schema`, for the legacy `x402@1.2.0` client the canon names and any agent parsing JSON rather than running an SDK.
  - **Acceptance** — `PAYMENT-SIGNATURE` (v2) and `X-PAYMENT` (v1); CDP `/verify` and `/settle` receive `paymentRequirements` serialised in **the version the client's payload declares**, read from the payload, never assumed.
  - **Settlement** — `PAYMENT-RESPONSE` (v2) or `X-PAYMENT-RESPONSE` (v1), base64 JSON carrying `transaction`, `network`, `payer`. The worker previously sent only the v2 name, unencoded — invisible to every client, which is why no one had noticed.
- **The rule that replaces "do not bump to v2".** One canonical requirements object per paid resource (`X402_RESOURCE_SPECS` + `x402Canonical()` in `src/index.ts`); every representation of the price is a projection of it — v2 header, v1 body, facilitator requirements, `/v5/pricing`, `/llms.txt`, the MCP server card, the A2A agent card, the key-delivery email — with a diff test that fails if any surface carries a literal. **Do not write a price, asset, `payTo`, network or resource URL as a literal anywhere else.** The version marker is no longer a thing anyone "bumps": both are served, and which one a client uses is the client's choice, read off its own payload.
- **What the tests do and do not establish.** They validate both representations against the **real** zod schemas, vendored byte-for-byte from `@x402/core` 2.20.0 into `test/vendor/x402-schemas.mjs` with digest and provenance, and a crossed-version test reproduces the 2026-06-07 `invalid_network` rejection in both directions. Both checks were driven red before being trusted. What they cannot establish is that CDP settles a real v2 payment, or that the 2.x client completes the round trip — no unit test reaches the facilitator or the chain. That is the live run, and it is a founder gate (D-21: `THROWAWAY_PK` set in the session shell, never in a file; the address funded with at least 0.001 USDC on Base).
- **The free trial gates any client test.** `/v5/status` serves three signed receipts per caller per day (reset 00:00Z) before it ever returns a 402. A compatibility test that does not exhaust the trial first is measuring a 200 and learning nothing about the payment path. Asserted in the suite so the next person does not rediscover it live.
- **Historical record kept.** The 2026-06-07 Bazaar settlement (tx `0xa6bc45dc8a1aa8652e59ca605b5a1adc1ee4f9c6197cf52fe2c4dc8cfe87ee41`, block 47,022,787) and the first dollar (tx `0xeb9da873…`, 2026-04-03) both settled under v1 and remain valid evidence of the payment path working end to end. What changed is which representation current clients read.

### Receipt coverage block — added to the signed payload 2026-09-07 (rail sprint T3)

- **What changed, under the spec-conformance guardrail.** A `coverage` field now sits INSIDE the Ed25519-signed payload of every receipt built by `buildSignedReceipt` — `/v5/demo`, `/v5/status`, `/v5/status/x402`, `/v5/batch`, `/v1/status/{MIC}` and the MCP `get_market_status` tool. **This is a change to the signed receipt format and it is flagged here deliberately**, per the guardrail in `CLAUDE.md`. What did NOT change: the signature algorithm, the canonicalization rule (alphabetical sort of top-level keys, `JSON.stringify` with no whitespace), the 60s TTL, the fail-closed tiers, or any existing field's name, type or meaning.
- **Why.** A verdict does not say what was looked at to reach it. "CLOSED" from the calendar and "CLOSED" because an operator tripped a circuit breaker are the same four characters, and neither tells a consumer whether an intraday halt would have been seen. For 26 of the 28 exchanges there is no intraday halt feed at all — an unscheduled halt on XLON is invisible to us. Until now the only hint was the two-valued `halt_detection` field. Now the scope we actually cover is signed, so a third party can hold us to what we claimed rather than what they assumed.
- **Shape.** `coverage` is a JSON-encoded **string** in the signed bytes — the convention `cross_venue` and `reasons` already use on the safe-to-trade receipt, and `exchanges` / `all_open` on `/v5/batch`. Consumers `JSON.parse` it after verifying the signature. It is a string and not a nested object on purpose: `signPayload` enforces string-only values, and a nested object would make its internal key ORDER load-bearing with no rule in the published spec saying so — which would be a real canonicalization change rather than a field addition.
  ```
  { determination_tier: 0 | 1 | 2,        // 0 = manual override (KV), 1 = schedule, 2 = fail-closed fallback
    consulted: [...], not_consulted: [...],
    realtime_halt_feed_scope: ["XNAS","XNYS"],
    unknown_reason: null | "UNSUPPORTED_MIC" | "NO_HOLIDAY_DATA_FOR_YEAR" | "DETERMINATION_ERROR" }
  ```
  The feed token is `realtime_halt_feed_via_override`, not `realtime_halt_feed`, because the receipt does **not** query a halt feed synchronously: the monitor runs on a one-minute cron and writes REALTIME entries into the override tier with a 2h TTL, and that is the only path by which a feed observation reaches a receipt. Naming it accurately is the point of the block.
- **Compatibility, stated plainly.** A verifier that builds the canonical payload from `/v5/keys → canonical_payload_spec` at runtime keeps working with no change. A verifier that hardcodes an older field list will return `INVALID_SIGNATURE` on every new receipt. Both halves were run against the published SDKs, not asserted: `@headlessoracle/verify@1.0.2` (npm) and `headless-oracle==0.1.1` (PyPI) each returned `valid: true` on a spec-driven verification of a real worker-served receipt carrying the block, and `INVALID_SIGNATURE` on both the stale field list and a receipt whose coverage block had been edited to claim the halt feed was consulted. Three in-repo verifier allowlists were updated in the same commit — that update **is** the compatibility contract, not a workaround for it.
- **Not closed.** `receipt-verify`'s `ho.receipt` adapter does not exist: version 0.1.2 declares four formats (`verification.*`, `acta.receipt/0`, `evidence.action/0`, `insight.attestation/eip712`) and none of them is HO's receipt. The DoD line "receipt-verify's ho.receipt adapter reports 0 unaddressed coverage items" cannot be run until that adapter is written, in a repository this sprint does not own. Live production receipts carrying the block were re-verified on 2026-09-07 by both published SDKs, spec-driven from live `/v5/keys`: `valid: true` on `/v5/demo?mic=XLON`, `?mic=XNYS` and the paid `/v5/status` receipt, `INVALID_SIGNATURE` on a tampered coverage block.

### GAP-021 — `/v5/payment-proof` KV anomalies (logged 2026-06-07)

- **Symptom**: public endpoint returns `first_payment_at: null`, `first_payment_tx: null` despite the April-3 first-dollar settlement (`0xeb9da873`) and the 2026-06-07 Bazaar settlement (`0xa6bc45dc…ee41`). `payment_count: 3` is likely an undercount of lifetime CDP + direct-on-chain settlements.
- **Likely cause**: the `x402_first_*` KV keys were either evicted, overwritten with `null`, or never populated for one of the settlement paths. `payment_count` increments live in two functions (`verifyX402Payment` direct-on-chain and `verifyX402ViaFacilitator` CDP) — a divergence in which one bumps which counter would explain the undercount.
- **Severity**: not load-bearing. No signing, settlement, or auth path depends on this KV. It's a diligence surface — surfaces lifetime x402 payment proof to investors / auditors. The actual settlement chain on-chain is intact and queryable.
- **Fix path (separate ticket, not blocking)**: walk `paddle_revenue_event:*`, `x402_used:*`, and `x402_used_tx:*` listable KV prefixes to reconstruct lifetime totals; then either (a) backfill the `x402_first_*` keys from the earliest event, or (b) compute the displayed counts on-read from the listable prefixes instead of relying on cached counters. Option (b) is more robust to future KV evictions.
...
- **Worker test environment fix — RESOLVED 2026-05-25** — the documented Supabase-DNS pre-commit hang did not reproduce. The 5xx fix sprint (logic commit `a43bb6b`) ran the full `.githooks/pre-commit` gate cleanly (tsc 0 / 1064 tests passed / `wrangler deploy --dry-run` 0). `getaddrinfo` for `sahqfuyneoeqczupmysu.supabase.co` now fails fast (`#11001 No such host`) rather than hanging 40+ min, so the swallowed best-effort writes no longer block the suite. Logic commits no longer need `--no-verify`.
