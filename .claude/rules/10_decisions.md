...
## ADR-019: Paddle billing — anonymous checkout, KV cache in front of Supabase, 402 for payment failures
**Decision**: Anonymous Paddle Checkout (no user accounts). `POST /v5/checkout` creates a Paddle transaction (subscription mode, `POST https://api.paddle.com/transactions`). `POST /webhooks/paddle` verifies `Paddle-Signature` header (format: `ts=<timestamp>;h1=<hex>`, signed content: `<timestamp>:<raw_body>`, HMAC-SHA256, 5-min replay window) before processing. Key format: `ok_live_<32 random hex bytes>`. Key hash: `sha256(UTF-8 encoded key string)` stored in Supabase `api_keys` table. `ORACLE_API_KEYS` KV caches `sha256(key) → { plan, status }` with 300s TTL. Auth hot path: MASTER → beta → KV hit → Supabase → 403. Suspended/cancelled keys return 402 (not 403) so agents can distinguish payment failure from invalid key. No Paddle SDK — pure `fetch()` calls against Paddle REST API. Secrets: `PADDLE_API_KEY`, `PADDLE_WEBHOOK_SECRET`, `PADDLE_PRICE_ID`.
...
**Paddle-specific decisions**: (1) `transaction.completed` fires on every successful payment including renewals — idempotency guard queries Supabase for existing `stripe_subscription_id` match before key generation; (2) Paddle's `transaction.completed` does not include customer email — fetched separately via `GET https://api.paddle.com/customers/{customer_id}`; (3) `stripe_customer_id` and `stripe_subscription_id` Supabase columns store Paddle `ctm_*` / `sub_*` IDs — naming mismatch only, no functional impact; (4) non-subscription `transaction.completed` events (no `subscription_id`) are silently skipped. Webhook events: `transaction.completed`, `subscription.updated`, `subscription.past_due`, `subscription.canceled`.
**Alternatives considered**: Supabase Auth for user accounts (adds friction, out of scope for V1), storing plaintext key in KV (security risk — KV is not a secret store), returning 403 for all payment failures (agents can't distinguish "rotate key" from "fix billing").
**Status**: Active — Feb 24 2026 (updated Feb 24 2026: Stripe → Paddle)
