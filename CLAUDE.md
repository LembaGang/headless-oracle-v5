...
- **Open follow-ups (post-calibration, 2026-06-04)**:
  ...
  - **GAP-020 — x402 surface is v1-only**: All three 402 emitters (`buildX402ScanPayload`, `buildMainnetFacilitatorPayload`, `buildX402IndexHeaders`) reverted to `x402Version: 1` in PR #21 (commit `12bb670`, worker `949607ae`, 2026-06-07). The latest published x402 npm client (`x402@1.2.0`) only emits v1-shape paymentPayloads; CDP `/verify` validates strictly against the declared version, and v2 `NetworkSchemaV2` rejects the bare `"base"` string the v1 client embeds (`invalid_network`). **Do NOT bump to v2 again until Coinbase publishes a v2-capable client to npm.** When that lands, the v2 migration is a coordinated change: CAIP-2 networks (`"eip155:8453"`) in both the 402 and `verifyX402ViaFacilitator`, `amount` instead of `maxAmountRequired`, top-level `resource: ResourceInfo`. Track upstream `x402` npm releases — `>= 2.x` likely signals v2 client support. Full GAP detail in `.claude/rules/90_active_priorities.md`.
